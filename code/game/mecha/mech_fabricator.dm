...
/obj/machinery/mecha_part_fabricator/Topic(href, href_list)
	...
	if(href_list["part_set"])
		var/tpart_set = tfilter.getStr("part_set")
		if(tpart_set)
			if(tpart_set=="clear")
				part_set = null
			else
	...
	if(href_list["add_to_queue"])
		var/T = tfilter.getStr("add_to_queue")
		var/datum/design/D = files.known_designs[T]
		if(D && D.build_type & MECHFAB)
			add_to_queue(D)
		return update_queue_on_page()
	if(href_list["remove_from_queue"])
		remove_from_queue(tfilter.getNum("remove_from_queue"))
		return update_queue_on_page()
	if(href_list["partset_to_queue"])
		add_part_set_to_queue(tfilter.get("partset_to_queue"))
		return update_queue_on_page()
	...
	if(href_list["queue_move"] && href_list["index"])
		var/index = tfilter.getNum("index")
		var/new_index = index + tfilter.getNum("queue_move")
	...
	if(href_list["part_desc"])
		var/T = tfilter.getStr("part_desc")
		var/datum/design/D = files.known_designs[T]
		if(D && D.build_type & MECHFAB)
			var/obj/part = D.build_path
