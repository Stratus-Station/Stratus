...
/obj/item/mecha_parts/mecha_equipment/medical/sleeper/Topic(href,href_list)
	...
	if(tfilter.get("eject"))
		go_out()
	if(tfilter.get("view_stats"))
		chassis.occupant << browse(get_patient_stats(),"window=msleeper")
		onclose(chassis.occupant, "msleeper")
		return
	if(tfilter.get("inject"))
		inject_reagent(tfilter.getType("inject",/datum/reagent),tfilter.getObj("source"))
	return
...
/obj/item/mecha_parts/mecha_equipment/medical/syringe_gun/Topic(href,href_list)
	..()
	var/datum/topic_input/tfilter = new (href,href_list)
	if(tfilter.get("toggle_mode"))
		mode = !mode
		update_equip_info()
		return
	if(tfilter.get("select_reagents"))
		...
		for(var/i=1 to known_reagents.len)
			if(m>=synth_speed)
				break
			var/reagent = tfilter.get("reagent_[i]")
		...
		return
	if(tfilter.get("show_reagents"))
		chassis.occupant << browse(get_reagents_page(),"window=msyringegun")
	if(tfilter.get("purge_reagent"))
		var/reagent = tfilter.get("purge_reagent")
		if(reagent)
			reagents.del_reagent(reagent)
		return
	if(tfilter.get("purge_all"))
