...
/obj/mecha/Topic(href, href_list)
	...
	if(usr.stat > 0)
		return
	var/datum/topic_input/tfilter = new /datum/topic_input(href,href_list)
	if(href_list["select_equip"])
		if(usr != occupant)	return
		var/obj/item/mecha_parts/mecha_equipment/equip = tfilter.getObj("select_equip")
		if(equip)
			selected = equip
	...
	if(href_list["rfreq"])
		if(usr != occupant)	return
		var/new_frequency = (radio.frequency + tfilter.getNum("rfreq"))
	...
	if(href_list["req_access"] && add_req_access)
		...
		output_access_dialog(tfilter.getObj("id_card"),tfilter.getMob("user"))
		return
	if(href_list["maint_access"] && maint_access)
		if(!in_range(src, usr))	return
		var/mob/user = tfilter.getMob("user")
		if(user)
			if(state==0)
				state = 1
			...
			output_maintenance_dialog(tfilter.getObj("id_card"),user)
		return
	...
	if(href_list["add_req_access"] && add_req_access && tfilter.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access += tfilter.getNum("add_req_access")
		output_access_dialog(tfilter.getObj("id_card"),tfilter.getMob("user"))
		return
	if(href_list["del_req_access"] && add_req_access && tfilter.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access -= tfilter.getNum("del_req_access")
		output_access_dialog(tfilter.getObj("id_card"),tfilter.getMob("user"))
		return
	...
	if(href_list["debug"])
		if(href_list["set_i_dam"])
			setInternalDamage(tfilter.getNum("set_i_dam"))
		if(href_list["clear_i_dam"])
			clearInternalDamage(tfilter.getNum("clear_i_dam"))
		return
	*/
